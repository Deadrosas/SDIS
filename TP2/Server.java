...
public class Server{
    ...
    public static void main(String [] args) throws IOException{
        ...
        int serverPort = getPort(args[0]);
        int multicastPort = getPort(args[2]);

        DatagramSocket multicastSocket = new DatagramSocket();
        InetAddress group = InetAddress.getByName(args[1]);
        sendResponse(multicastSocket, args[0], group, multicastPort);

        DatagramSocket serverSocket = new DatagramSocket(null);
        serverSocket.bind(new InetSocketAddress(InetAddress.getByName("localhost"),Integer.valueOf(serverPort)));
        
        startMulticasting(serverSocket);
    ...
    private static void startMulticasting(DatagramSocket socket){
